...
# 직책별 질문 축. API 서버의 role enum 5종(TECH/HR/CEO/PM/DESIGN) 을 전부 덮는다.
# 알려진 직책은 여기서 관점을 고정하고, 모르는 직책은 아래 기본 지침으로 간다.
# TECH 는 보통 techPersona 슬롯이 맡지만, otherPersonas 로 두 번째 기술 면접관이 올 수도 있다.
# role 문자열은 호출자가 자유롭게 넣을 수 있으므로(새 직책 추가 시 422 를 피하려고)
# 대소문자를 무시하고 맞춘다.
_ROLE_GUIDANCE: dict[str, str] = {
    "tech": (
        "구현의 근거와 깊이를 본다. 왜 그 구조와 기술을 골랐는지, 어디서 막혔고 어떻게 풀었는지. "
        "프로젝트 요약에 근거가 없는 세부 구현은 묻지 않는다."
    ),
    "hr": (
